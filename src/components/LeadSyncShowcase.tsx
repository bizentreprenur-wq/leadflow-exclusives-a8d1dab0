...
export default function LeadSyncShowcase() {
  ...
  const { user } = useAuth();
  const [processingTier, setProcessingTier] = useState<LeadSyncTier | null>(null);
  const [showPricingModal, setShowPricingModal] = useState(false);

  const handleSubscribe = async (tier: LeadSyncTier) => {
    // Check if user is logged in
    if (!user) {
      toast.info('Please sign in to subscribe');
      navigate(`/auth?plan=${tierToStripePlan[tier]}&billing=monthly&redirect=dashboard`);
      return;
    }
    ...
    try {
      const { checkout_url } = await createCheckoutSession(tierToStripePlan[tier], 'monthly');
        ...
        <div className="container px-4 relative z-10">
          ...
          >
            ...
            <div>
              ...
              >
                View full feature comparison <ArrowRight className="w-4 h-4 ml-1" />
              </Button>
            </div>
          </motion.div>
        </div>
