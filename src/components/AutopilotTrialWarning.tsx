...
export default function AutopilotTrialWarning({
  ...
  // Active trial warning - varies by urgency
  if (variant === 'inline') {
    return (
      <Badge className={cn(badgeColor, "text-xs gap-1.5", className)}>
    ...
    return (
      <motion.div
        ...
        className={cn(
          ...
          status.warningLevel === 'high' && "bg-red-500/10 border border-red-500/30",
          status.warningLevel === 'medium' && "bg-amber-500/10 border border-amber-500/30",
          (status.warningLevel === 'low' || status.warningLevel === 'none') && "bg-emerald-500/10 border border-emerald-500/30",
          className
        )}
      >
        <div className={cn(
          ...
          (status.warningLevel === 'low' || status.warningLevel === 'none') && "bg-emerald-500/20",
        )}>
          <Clock className={cn(
            "w-4 h-4",
            status.warningLevel === 'high' && "text-red-400",
            status.warningLevel === 'medium' && "text-amber-400",
            (status.warningLevel === 'low' || status.warningLevel === 'none') && "text-emerald-400",
          )} />
        </div>
        <div className="flex-1 min-w-0">
          <p className={cn(
  ...
  return (
    <motion.div
      ...
      className={cn(
        ...
        status.warningLevel === 'medium' && "bg-gradient-to-r from-amber-500/10 to-orange-500/10 border-amber-500/30",
        (status.warningLevel === 'low' || status.warningLevel === 'none') && "bg-gradient-to-r from-emerald-500/10 to-teal-500/10 border-emerald-500/30",
        className
      )}
    >
      {onDismiss && (
          ...
          <div className="mb-2">
            <Progress 
              ...
              className={cn(
                ...
                (status.warningLevel === 'low' || status.warningLevel === 'none') && "[&>div]:bg-emerald-500",
              )}
            />
          </div>
